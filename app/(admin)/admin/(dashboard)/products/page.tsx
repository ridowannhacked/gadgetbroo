...
export default function ProductsPage() {
  ...
  // Other filters (could also be moved to URL)
  ...
  const updateFilter = (key: string, value: string) => {
    // For simplicity, we just reset the page URL parameter when local filters change
        ...
        <div className="bg-card/90 border border-border rounded-xl p-4 sm:p-6 space-y-6 shadow-2xl backdrop-blur-md">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative min-w-[150px]">
              <select
                value={categoryId}
                onChange={(e) => { setCategoryId(e.target.value); updateFilter('categoryId', e.target.value); }}
            ...
            <div className="relative min-w-[140px]">
              <select
                value={featured}
                onChange={(e) => { setFeatured(e.target.value); updateFilter('featured', e.target.value); }}
                className="w-full bg-background border border-border text-foreground text-sm rounded-lg px-3 py-2.5 pr-8 appearance-none outline-none focus:border-border cursor-pointer"
              >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
            <div className="flex items-center gap-6 text-sm">
              {(["all", "in", "low", "out"] as const).map((tab) => (
                <button
                  key={tab}
                  onClick={() => { setActiveTab(tab); updateFilter('tab', tab); }}
